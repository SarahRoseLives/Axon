				...
				for peerID, msgs := range pendingMap {
					if len(msgs) > 0 {
						go func(pid string, m chat.Message) {
							...
							// Attempt send and log the result
							success := AttemptSendMessage(torCtrl, pm, chatMgr, pid, m, getNickname(), identityKey)

							<-TorLimiter

							// Verbose Loop Logging
							if success {
								fmt.Printf("✅ Outbox: Message %s delivered to %s\n", m.ID, pid)
							} else {
								fmt.Printf("⚠️ Outbox: Failed to deliver %s to %s (Will retry)\n", m.ID, pid)
							}
						}(peerID, msgs[0])
					}
				}
...
func AttemptSendMessage(torCtrl *tor.Controller, pm *discovery.PeerManager, chatMgr *chat.Manager, targetPeer string, msg chat.Message, myNickname string, privKey ed25519.PrivateKey) bool {
	...
	// 1. Check for Public Key
	if peerKeyHex == "" {
		fmt.Printf("❌ [DEBUG] Send Failed: No public key for %s. Triggering handshake.\n", targetPeer)
		go func() {
			TorLimiter <- struct{}{}
	...
	if err != nil {
		fmt.Printf("❌ [DEBUG] Encryption Error: %v\n", err)
		return false
	}
	...
	client, err := torCtrl.GetHttpClient()
	if err != nil {
		fmt.Printf("❌ [DEBUG] Tor Client Error: %v\n", err)
		return false
	}

	url := fmt.Sprintf("http://%s/api/chat/recv", targetPeer)
	fmt.Printf("📤 [DEBUG] Sending message to %s...\n", url)
	...
	if err == nil {
		...
		if resp.StatusCode == 200 {
			...
			// Read error body from Flutter/Peer to understand rejection
			body, _ := io.ReadAll(resp.Body)
			fmt.Printf("❌ [DEBUG] Remote Server Error: [%d] %s\n", resp.StatusCode, string(body))
		}
	} else {
